...
resource "aws_iam_role" "lambda_s3_trigger" {
    name = "${var.org}-${var.app}-${var.env}-lambda-s3-trigger"
...
module "lambda_s3_trigger" {
    ...
    function_name = "${var.org}-${var.app}-${var.env}-s3-trigger"
    ...
    environment_variables = {
        PIPELINE_NAME = aws_codepipeline.terraform_plan.name
    }
...
resource "aws_lambda_permission" "s3_trigger" {
    ...
    source_arn    = aws_s3_bucket.terraform_artifacts.arn
}
